...
class Exp_Main(Exp_Basic):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if not is_test:
                    ...
                    weights = [math.atan(1.0 / i) + math.pi / 4 for i in range(1, T + 1)]
                    self.ratio = torch.tensor(weights, dtype=torch.float32).unsqueeze(-1).to('cuda')
                    ...
                    hp_ratio  = 0.25            # 25%
                    lam_freq  = 0.05            # 权重系数
                    E_fft = torch.fft.rfft(outputs - batch_y, dim=1)   # [B,F,C] 复数
                    Fbins = E_fft.size(1)
                    k0 = int(Fbins * hp_ratio)
                    loss_freq = torch.mean(torch.abs(E_fft[:, k0:, :]))

                    # 3) 组合
                    loss = loss + lam_freq * loss_freq    
                else:
    ...
    def train(self, setting):
        ...
        scheduler = ReduceLROnPlateau(model_optim, mode='min', factor=0.3, patience=self.args.time, verbose=True, min_lr=5e-7)
